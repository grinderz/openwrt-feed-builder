...
// Magic prefixes of the archive formats an .ipk may be built from.
const (
	arMagic   = "!<arch>\n"
	gzipMagic = "\x1f\x8b"
	xzMagic   = "\xfd7zXZ\x00"
...
func readArMembers(data []byte) (map[string][]byte, error) {
	if !hasMagic(data, arMagic) {
		return nil, fmt.Errorf("%w: not an ar archive", errIPK)
	}
...
func extractMemberFromTar(blob []byte, wanted string) ([]byte, error) {
	...
	for {
		...
		if normalized == wanted {
			return readAll(tarReader)
		}
	}

	return nil, fmt.Errorf("%w: %q not found in tar archive", errIPK, wanted)
}
...
func readControlBytes(data []byte, name string) (string, error) {
	if hasMagic(data, arMagic) {
		...
		if controlTar == nil {
			return "", fmt.Errorf("%w: no control.tar.* inside %s", errIPK, name)
		}
	...
	return "", fmt.Errorf("%w: unrecognized format: %s", errIPK, name)
}
...
func parseFields(controlText string) map[string]string {
	...
	for line := range strings.SplitSeq(controlText, "\n") {
		...
		if before, after, ok := strings.Cut(line, ":"); ok {
			...
			fields[key] = strings.TrimSpace(after)
		}
	}
